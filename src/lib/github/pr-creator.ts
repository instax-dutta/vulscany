...
async function hasExistingSecurityPR(
    ...
    try {
        ...
        // Look for PRs created by vulscany or older legacy branches
        const existingPR = prs.find(pr =>
            pr.head.ref.startsWith('vulscany/security-fixes-') ||
...
export async function createSecurityFixPR(
    ...
    const branchName = `vulscany/security-fixes-${timestamp}`;
    ...
    try {
        ...
        try {
            await octokit.issues.addLabels({
                ...
                repo,
                issue_number: number,
                labels: ['security', 'vulscany', 'automated']
            });
        } catch (error) {
...
`;
    ...
    return `## Security Fixes by vulscany

This PR contains automated security fixes for vulnerabilities detected by vulscany.
...
**Generated by:** vulscany archive tooling
