...
export function loadCommunityPatterns(): CommunityPattern[] {
    ...
    const stored = localStorage.getItem('aeglyn_community_patterns');
    if (stored) {
        try {
...
export function saveCommunityPatterns(patterns: CommunityPattern[]): void {
    ...
    localStorage.setItem('aeglyn_community_patterns', JSON.stringify(custom));
}
...
export function voteForPattern(patternId: string): void {
    ...
    // Track user votes
    const votedKey = 'aeglyn_voted_patterns';
...
export function hasVotedFor(patternId: string): boolean {
    ...
    const voted = JSON.parse(localStorage.getItem('aeglyn_voted_patterns') || '[]');
