...
|--------|--------------|-------------|
| **Domain** | `aeglyn.site` | `app.aeglyn.site` |
| **Repository** | `aeglyn-landing-v1` | `vulscany` |
...
1. NEW USER DISCOVERS AEGLYN
   ...
   ├─ Finds aeglyn.site via Google/social media
   └─ Lands on: https://aeglyn.site
        │
        ▼
...
3. USER CLICKS "LAUNCH APP" OR "GET STARTED"
   ├─ Button links to: https://app.aeglyn.site
...
4. APP CHECKS AUTHENTICATION
   ...
   └─ App redirects BACK to: https://aeglyn.site
        │
        │  ┌─────────────────────────────────────┐
        │  │  WHY REDIRECT BACK?                 │
...
6. GITHUB OAUTH FLOW
   ...
   └─ GitHub redirects to: app.aeglyn.site/api/auth/callback
        │
        ▼
...
export function GitHubAuthButton() {
  const handleGitHubAuth = () => {
    const clientId = 'Ov23liYeCOyTjZbOGnoy'; // Your GitHub OAuth Client ID
    const redirectUri = encodeURIComponent('https://app.aeglyn.site/api/auth/callback');
...
**Critical Details**:
- **Client ID**: `Ov23liYeCOyTjZbOGnoy`
- **Redirect URI**: `https://app.aeglyn.site/api/auth/callback`
...
<nav>
  ...
  <Link href="https://app.aeglyn.site">Dashboard</Link> {/* Links to app */}
</nav>
```
...
<footer>
  <div className="footer-links">
    <Link href="https://aeglyn.site/privacy">Privacy Policy</Link>
    <Link href="https://aeglyn.site/terms">Terms of Service</Link>
    <Link href="https://app.aeglyn.site">Launch App</Link>
  </div>
</footer>
```
...
#### Root Route Behavior (`app.aeglyn.site/`)

**File**: `src/app/page.tsx`

**Logic**:
```typescript
1. User visits app.aeglyn.site
    ...
    const checkAuth = async () => {
      try {
        ...
        if (session?.user) {
          ...
          window.location.href = 'https://aeglyn.site';
        }
      } catch (error) {
        // Error → Landing page
        window.location.href = 'https://aeglyn.site';
...
│    └─ If not authenticated   → https://aeglyn.site           │
│                                                               │
...
├─ Location: aeglyn.site
├─ Button triggers JavaScript function
└─ Constructs OAuth URL with:
    ├─ client_id: Ov23liYeCOyTjZbOGnoy
    ├─ redirect_uri: https://app.aeglyn.site/api/auth/callback
...
├─ GitHub redirects to: app.aeglyn.site/api/auth/callback?code=ABC123
...
Time: 0:31
├─ Browser navigates to: app.aeglyn.site
├─ App shows loading spinner
└─ App checks authentication: NONE

Time: 0:32
├─ App redirects back to: aeglyn.site
...
├─ Browser goes to: app.aeglyn.site
├─ App detects session
  ...
  return (
    <section className="hero">
      ...
      <p className="text-sm text-muted">
        Already have access? <a href="https://app.aeglyn.site">Launch App</a>
      </p>
    </section>
  );
}
...
□ Visit aeglyn.site
□ Click "Launch App"
□ Should redirect back to aeglyn.site
□ Click "Connect with GitHub"
□ Authorize on GitHub
□ Should land on app.aeglyn.site/dashboard
...
□ Authorize
□ Verify callback URL: app.aeglyn.site/api/auth/callback
...
□ Click link to aeglyn.site (if any)
...
```tsx
// ❌ WRONG
redirect_uri: 'https://aeglyn.site/callback'
...
```tsx
// ❌ WRONG
redirect_uri: 'http://app.aeglyn.site/api/auth/callback'
...
// ✅ CORRECT
<Link href="https://app.aeglyn.site/dashboard">
```
...
export const SITE_CONFIG = {
  ...
  github: {
    clientId: process.env.NODE_ENV === 'production'
      ? 'Ov23liYeCOyTjZbOGnoy'  // Production
...
```
Landing Page:     https://aeglyn.site
Application:      https://app.aeglyn.site
OAuth Callback:   https://app.aeglyn.site/api/auth/callback
Dashboard:        https://app.aeglyn.site/dashboard
```
...
Scopes:           read:user repo
Callback URL:     https://app.aeglyn.site/api/auth/callback
```
...
✓ Proper href attributes (https://app.aeglyn.site)
```
...
- Landing: `https://aeglyn.site`
- App: `https://app.aeglyn.site`
