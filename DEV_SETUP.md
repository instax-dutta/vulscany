...
- Deployment: `app.example.com`
- Managed: On this device

**Landing Page** (Different Device)
- Repository: `vulscany-landing-v1` (GitHub: instax-dutta/vulscany-landing-v1)
...
# Test locally
npm run dev
# Landing page will be in vulscany-landing/ folder
```

### After Testing
The `vulscany-landing/` directory is gitignored, so it won't be committed.
You can safely delete it after testing:
```powershell
Remove-Item -Recurse -Force vulscany-landing
```
...
From `vulscany-landing/public/` to `public/`:
...
- Push to `vulscany-landing-v1` repo
- Auto-deploys to `example.com`

**Main App**
- Edit on this device
- Push to `vulscany` repo
- Auto-deploys to `app.example.com`
