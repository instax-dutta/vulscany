...
- **Repository**: `vulscany-landing/` (cloned from instax-dutta/vulscany-landing-v1)
- **Deployment**: Vercel → `example.com`
...
- **Deployment**: Vercel → `app.example.com`
...
```tsx
// In vulscany-landing components
<Link href="https://app.example.com">Get Started</Link>
<Link href="https://app.example.com/dashboard">Dashboard</Link>
```
...
```
vulscany/
├── vulscany-landing/          # Marketing site (separate deployment)
│   ├── app/
│   ├── components/
│   ├── public/
│   └── styles/
...
### DNS Setup
```
example.com          → Vercel (landing page)
www.example.com      → Redirect to example.com
app.example.com      → Vercel (application)
```
...
2. **Domain Redirect**: Set up vullscanny.sdad.pro → app.example.com
